  ...
  return (
    <>
      <header className="absolute inset-x-0 top-0 z-50 ">
        <nav
          aria-label="Global"
          className="flex items-center justify-between p-[3rem] mx-4  lg:px-8"
        >
          ...
          <div className="hidden lg:flex lg:gap-x-12">
            {nav.map((item: any) => (
              <a
                key={item.name}
                href={item.href}
                className="text-0.9rem hover:text-accent transition-all duration-400 ease-in-out  font-poppins font-semibold leading-6 text-gray-900"
              >
                {item.name}
              </a>
            ))}
          ...
          <div className="hidden lg:flex lg:flex-1 lg:justify-end ">
            ...
            >
              ...
              <span className="relative w-full text-left text-textPrimary transition-colors duration-200 ease-in-out group-hover:text-gray-900">
                Enroll Now
              </span>
          ...
          <DialogPanel className="fixed inset-y-0 right-0 z-50 w-full overflow-y-auto bg-white px-6 py-6 sm:max-w-sm sm:ring-1 sm:ring-gray-900/10">
            <div className="flex items-center justify-between">
              <a href="#" className="-m-1.5 p-1.5">
                ...
                <img alt="" src={"logo.png"} className="h-[5.5rem] w-auto" />
              </a>
              <button
                type="button"
