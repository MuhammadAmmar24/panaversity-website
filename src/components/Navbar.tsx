...
export default function Navbar() {
  ...
  return (
    ...
    <header className="absolute inset-x-0 top-0 z-50 ">
        <nav aria-label="Global" className="flex items-center justify-between p-[3rem] mx-4  lg:px-8">
          <div className="flex lg:flex-1">

            <a href="#" className="-m-1.5 p-1.5 cursor-pointer mt-[-2.9rem]">
              <img
                alt=""
                src={'logo.png'}
          ...
          <div className="hidden lg:flex lg:gap-x-12">
            {nav.map((item : any) => (
              <a key={item.name} href={item.href} className="text-0.9rem hover:text-accent transition-all duration-400 ease-in-out  font-poppins font-semibold leading-6 text-gray-900">
                {item.name}
              </a>
            ))}
          </div>
...
<span className="relative w-full text-left text-textPrimary transition-colors duration-200 ease-in-out group-hover:text-gray-900">Enroll Now</span>
